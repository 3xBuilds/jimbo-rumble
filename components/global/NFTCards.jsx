...
const NFTCard = ({id, username, nftname, type, image, setChosen, selected, joinBattle, entryFee, currency}) => {
  ...
  if(type==="user" && selected) return (
    <div className={`border-[1px] border-jimbo-green flex flex-col justify-between rounded-lg w-52 min-w-52 p-2 pb-0 bg-jimbo-green/30`}>
        ...
        <h3 className="text-white text-center mt-2 leading-none">{entryFee} <span className="text-jimbo-green">{currency}</span></h3>
        <button onClick={()=>{
          joinBattle();
